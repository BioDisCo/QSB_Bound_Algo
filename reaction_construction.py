...
import rate_function_scripts as rf
import itertools
        ...
        for combination_of_reactant_species in iterator_for_combinations(reactant_species_combination_list):
            ...
            for reactant_string_list in iterator_for_combinations(reactant_species_string_combination_list):
                ...
                for product_string_list in iterator_for_combinations(product_species_species_string_combination_list):
                    ...
                    reacting_meta_species = [x['object'] for x in combination_of_reactant_species]

                    try:
                        rate_function, argument_number = rf.create_rate_function(reacting_meta_species,
                                                                                 reactant_strings,
                                                                                 reaction.rate, dimension)
                    except TypeError as e:
                        simlog.error(f'On reaction {reaction} \n' + str(e))

                    reactant_count = count_string_dictionary(reactant_strings)
                    product_count = count_string_dictionary(product_string_list)
                    delta_species = subtract_count_dictionaries(reactant_count, product_count)
                    compiled_reaction = {'reactants': reactant_strings,
                                         'delta_species': delta_species,
                                         'rate_function': rate_function,
                                         'argument_number': argument_number}
                    reactions_for_q[i] = compiled_reaction
                    i = i + 1

    expression_compilation_finish(expressions_in_stack)

    return reactions_for_q


def subtract_count_dictionaries(reactant_count_dict, product_count_dict):
    difference_dictionary = {}

    for r in reactant_count_dict:
        difference_dictionary[r] = -reactant_count_dict[r]

    for p in product_count_dict:
        try:
            difference_dictionary[p] = difference_dictionary[p] + product_count_dict[p]
        except KeyError:
            difference_dictionary[p] = product_count_dict[p]
