...
def calculate_distribution(d, t):
    Q_dense = Q_full.toarray()  # Convert sparse to dense
...
n_variables = Q_full.shape[0]

# Plotting
fig, axes = plt.subplots(2, 2, figsize=(10, 8))

d_minus = np.zeros(n_variables)
d_minus[0] = 1
d_minus = calculate_distribution(d_minus, 1000)

# Plot 1
axes[0, 0].plot([i for i in range(n_variables)], d_minus, label=r'$d_{H0}$')
...
for t in [1, 1e4, 1e5, 1e6]:
    dif_minus = np.zeros(n_variables)
    d_minus = np.zeros(n_variables)
    d_minus[0] = 1
    ...
    axes[0, 1].plot([i for i in range(n_variables)], dif_minus, label=f't={t} (h)')

axes[0, 1].set_title(r"Difference $\pi_{-}$ and $d_{H0}$")
axes[0, 1].set_xlabel("H (count)")
axes[0, 1].set_ylabel("State Probability")

d_plus = np.zeros(n_variables)
d_plus[P] = 1
d_plus = calculate_distribution(d_plus, 1000)

# Plot 3
axes[1, 0].plot([i for i in range(n_variables)], d_plus, label=r'$d_{HP}$')
...
for t in [500, 5e9, 1e12]:
    ...
    for i in range(n_variables):
        try:
            ...
            dif_plus[-i] = d_plus[-i]
    axes[1, 1].plot([i for i in range(n_variables)], dif_plus, label=f't={t} (h)')
